...
- [-] Suggestion of link reference IDs by fuzzy matching on spec text and ID tag
  ...
  - [ ] Native support
* Tasks
** TODO Add linkification of md files
...
- [-] Use kuchiki to rewrite unlinked tag refs to links
  + [X] Select text nodes
  + [ ] Check if a =text node= contains a tag-ref
    - [ ] Parse text into a vec of enums ~Text(String) | UnitRef(String)~
    - [ ] Generate a =Ref link= for each ~UnitRef~
      + [ ] Get location by querying the db for locations of logical units.
      + [ ] Create a link element
    - [ ] Add =Before ref= after text node
    - [ ] Add =Ref link= after text node
    - [ ] Add =After ref=
    - [ ] Remove =text node=
- [ ] Use pandoc to convert HTML back to markdown, using the =--reference-links=
  flag.
- [ ] Add CLI to call command
