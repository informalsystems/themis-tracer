...
- [ ] Rework anchoring to support gfm
  + [ ] Turn pandoc::write_file to pandoc::html_to_pandoc: String -> String
  + [ ] Use flag to trigger gfm compat
  + [ ] On linkification, don't anchor in html when gfm is true
  + [ ] Implement simple regex rule to anchor tags
** TODO Actually fetch upstream git data
- [ ] Compute the url for linkification from the remote git URL (specific to github to start)
